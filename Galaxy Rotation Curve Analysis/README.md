...
Δt(L) = Δt\_0 \\left\[ 1 + (L/L\_c)^p \\right]^q.
...
\### \*\*1. `phase22B\_tfgr\_batch\_fit.py`\*\*
...
\### \*\*2. `phase23C\_timefield\_mass\_scaling.py`\*\*
...
\### \*\*3. `phase23D\_visualize\_meff\_scaling.py`\*\*
...
File: `TFGR\_AIC\_BIC\_comparison.png`
...
File: `TFGR\_vs\_LCDM\_Meff\_scaling.png`



\- TFGR reproduces a natural \\( M\_{\\text{eff}} \\propto L^{\\alpha} \\) scaling  

&nbsp; emerging from the time-field potential  

\- LCDM requires separate free halo parameters per galaxy  

\- TFGR explains data with a \*single universal scale\* \\( L\_c ≈ 4×10^9\\,\\mathrm{m} \\)
...
\- `r\_kpc` – radial distance  

\- `v\_obs` – observed rotation velocity  

\- `v\_err` – measurement uncertainty  
